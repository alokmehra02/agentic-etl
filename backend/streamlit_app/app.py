...
if job_id:
    ...
    c3.metric("Quality", detail.get("taste_profile", {}).get("quality_score", 0) if detail.get("taste_profile") else 0)

    if detail.get("agent_runs"):
        st.markdown("**Agent run logs**")
        for run in detail["agent_runs"]:
            st.json({"thread_id": run["thread_id"], "status": run["status"], "step": run["current_step"], "logs": run.get("logs")})
